...
async function geocodeLocations(locations) {
  ...
  // On errors, we write a default value so the destructuring works below
  geocodedResults.forEach(g => {
    if (g.error || Array.isArray(g.value) && g.value.length === 0) {
      g.value = [{latitude: Infinity, longitude: Infinity}]
      g.error = g.error && g.error.message
    }
  })
