/**
 * Matcher.js contains the algorithms used to find the closest points.
 *
 * Algorithms:
 *
 *   - bruteforce: O(n^2) loops over all coordinates
 *
 *   - delaunay: O(n*logn) builds a graph via delaunay from coordinates, then inspects the distance to each nodes connected neighbors
 *
 *   - sortedList: (not implemented) - the idea was to do a recursive bisecting search to find the closest nodes via inspecting rectangular windows
 *
 */
 ...
 * matcher
 *
 * API used by index.js to find matches
 */
async function matcher (coordinateList) {
  // if (coordinateList.length < 45) {
